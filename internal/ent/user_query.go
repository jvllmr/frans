...
func (_q *UserQuery) QueryGrants() *GrantQuery {
...
// QueryFileinfos chains the current query on the "fileinfos" edge.
func (_q *UserQuery) QueryFileinfos() *FileDataQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFileinfos: _q.withFileinfos.Clone(),
...
func (_q *UserQuery) WithGrants(opts ...func(*GrantQuery)) *UserQuery {
	query := (&GrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithFileinfos tells the query-builder to eager-load the nodes that are connected to
// the "fileinfos" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithFileinfos(opts ...func(*FileDataQuery)) *UserQuery {
	query := (&FileDataClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFileinfos = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withGrants; query != nil {
		if err := _q.loadGrants(ctx, query, nodes,
			func(n *User) { n.Edges.Grants = []*Grant{} },
			func(n *User, e *Grant) { n.Edges.Grants = append(n.Edges.Grants, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFileinfos; query != nil {
		if err := _q.loadFileinfos(ctx, query, nodes,
			func(n *User) { n.Edges.Fileinfos = []*FileData{} },
			func(n *User, e *FileData) { n.Edges.Fileinfos = append(n.Edges.Fileinfos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*User, init func(*User), assign func(*User, *Ticket)) error {
	...
	for _, n := range neighbors {
		fk := n.user_tickets
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_tickets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_tickets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadGrants(ctx context.Context, query *GrantQuery, nodes []*User, init func(*User), assign func(*User, *Grant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Grant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_grants
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_grants" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_grants" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadFileinfos(ctx context.Context, query *FileDataQuery, nodes []*User, init func(*User), assign func(*User, *FileData)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.FileinfosTable)
		s.Join(joinT).On(s.C(filedata.FieldID), joinT.C(user.FileinfosPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.FileinfosPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.FileinfosPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*FileData](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "fileinfos" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
