...
func (_q *FileQuery) QueryTicket() *TicketQuery {
...
func (_q *FileQuery) QueryGrant() *GrantQuery {
...
func (_q *FileQuery) WithTicket(opts ...func(*TicketQuery)) *FileQuery {
	query := (&TicketClient{config: _q.config}).Query()
...
func (_q *FileQuery) WithGrant(opts ...func(*GrantQuery)) *FileQuery {
	query := (&GrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if _q.withTicket != nil || _q.withGrant != nil || _q.withOwner != nil || _q.withData != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTicket; query != nil {
		if err := _q.loadTicket(ctx, query, nodes, nil,
			func(n *File, e *Ticket) { n.Edges.Ticket = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGrant; query != nil {
		if err := _q.loadGrant(ctx, query, nodes, nil,
			func(n *File, e *Grant) { n.Edges.Grant = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *FileQuery) loadTicket(ctx context.Context, query *TicketQuery, nodes []*File, init func(*File), assign func(*File, *Ticket)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*File)
	for i := range nodes {
		if nodes[i].ticket_files == nil {
			continue
		}
		fk := *nodes[i].ticket_files
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ticket_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *FileQuery) loadGrant(ctx context.Context, query *GrantQuery, nodes []*File, init func(*File), assign func(*File, *Grant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*File)
	for i := range nodes {
		if nodes[i].grant_files == nil {
			continue
		}
		fk := *nodes[i].grant_files
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "grant_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
