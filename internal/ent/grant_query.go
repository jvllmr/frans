...
func (_q *GrantQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *File)) error {
	...
	query.Where(predicate.File(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(grant.FilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.grant_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "grant_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "grant_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
