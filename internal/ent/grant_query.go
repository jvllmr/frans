...
func (_q *GrantQuery) QueryFiles() *FileQuery {
...
func (_q *GrantQuery) QueryOwner() *UserQuery {
...
func (_q *GrantQuery) QueryShareaccesstokens() *ShareAccessTokenQuery {
	query := (&ShareAccessTokenClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GrantQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *GrantQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *GrantQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *GrantQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *GrantQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *GrantQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *GrantQuery) Clone() *GrantQuery {
	...
	return &GrantQuery{
		...
		withShareaccesstokens: _q.withShareaccesstokens.Clone(),
...
func (_q *GrantQuery) WithFiles(opts ...func(*FileQuery)) *GrantQuery {
	query := (&FileClient{config: _q.config}).Query()
...
func (_q *GrantQuery) WithOwner(opts ...func(*UserQuery)) *GrantQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GrantQuery) WithShareaccesstokens(opts ...func(*ShareAccessTokenQuery)) *GrantQuery {
	query := (&ShareAccessTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withShareaccesstokens = query
	return _q
}
...
func (_q *GrantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Grant, error) {
	...
	if query := _q.withFiles; query != nil {
		if err := _q.loadFiles(ctx, query, nodes,
			func(n *Grant) { n.Edges.Files = []*File{} },
			func(n *Grant, e *File) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Grant, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withShareaccesstokens; query != nil {
		if err := _q.loadShareaccesstokens(ctx, query, nodes,
			func(n *Grant) { n.Edges.Shareaccesstokens = []*ShareAccessToken{} },
			func(n *Grant, e *ShareAccessToken) { n.Edges.Shareaccesstokens = append(n.Edges.Shareaccesstokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GrantQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *File)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Grant)
	nids := make(map[uuid.UUID]map[*Grant]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(grant.FilesTable)
		s.Join(joinT).On(s.C(file.FieldID), joinT.C(grant.FilesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(grant.FilesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(grant.FilesPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Grant]struct{}{byID[outValue]: {}}
...
func (_q *GrantQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Grant)
	for i := range nodes {
		if nodes[i].user_grants == nil {
			continue
		}
		fk := *nodes[i].user_grants
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_grants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *GrantQuery) loadShareaccesstokens(ctx context.Context, query *ShareAccessTokenQuery, nodes []*Grant, init func(*Grant), assign func(*Grant, *ShareAccessToken)) error {
	...
	query.Where(predicate.ShareAccessToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(grant.ShareaccesstokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.grant_shareaccesstokens
		if fk == nil {
			return fmt.Errorf(`foreign-key "grant_shareaccesstokens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "grant_shareaccesstokens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *GrantQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(grant.Table, grant.Columns, sqlgraph.NewFieldSpec(grant.FieldID, field.TypeUUID))
	_spec.From = _q.sql
