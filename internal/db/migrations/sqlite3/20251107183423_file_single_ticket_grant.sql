...
CREATE TABLE `new_files` (
  ...
  `grant_files` uuid NULL,
  `ticket_files` uuid NULL,
  ...
  CONSTRAINT `files_tickets_files` FOREIGN KEY (`ticket_files`) REFERENCES `tickets` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
  CONSTRAINT `files_grants_files` FOREIGN KEY (`grant_files`) REFERENCES `grants` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
...
INSERT INTO `new_files` (`id`, `name`, `created_at`, `last_download`, `times_downloaded`, `expiry_type`, `expiry_total_days`, `expiry_days_since_last_download`, `expiry_total_downloads`, `file_data`, `user_files`) SELECT `id`, `name`, `created_at`, `last_download`, `times_downloaded`, `expiry_type`, `expiry_total_days`, `expiry_days_since_last_download`, `expiry_total_downloads`, `file_data`, `user_files` FROM `files`;
