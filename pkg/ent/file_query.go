...
func (fq *FileQuery) QueryTickets() *TicketQuery {
	query := (&TicketClient{config: fq.config}).Query()
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withTickets: fq.withTickets.Clone(),
...
func (fq *FileQuery) WithTickets(opts ...func(*TicketQuery)) *FileQuery {
	query := (&TicketClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withTickets = query
	return fq
}
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withTickets; query != nil {
		if err := fq.loadTickets(ctx, query, nodes,
...
func (fq *FileQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*File, init func(*File), assign func(*File, *Ticket)) error {
